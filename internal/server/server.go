...
func NewServer(config *config.Config, logger *zap.Logger, sqlDB *sql.DB, fiberApp *fiber.App) *server {
	dbRepo, err := dbrepo.NewDatabaseRepository(sqlDB)
	if err != nil {
		logger.Fatal("Failed to create database repository", zap.Error(err))
	}

	services := service.NewServices(dbRepo)
